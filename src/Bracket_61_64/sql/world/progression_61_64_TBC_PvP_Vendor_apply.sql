...
UPDATE `creature_template` SET `subname` = 'S1竞技场商人', `npcflag`=`npcflag`|128 WHERE (`entry` = @NPC_SEASON_ONE);
SET @NPC_SEASON_TWO          := 26352; -- Big Zokk Torquewrench
UPDATE `creature_template` SET `subname` = 'S2竞技场商人', `npcflag`=`npcflag`|128 WHERE (`entry` = @NPC_SEASON_TWO); -- If NPC is changed remember to set the correct subname later
SET @NPC_SEASON_THREE        := 26378; -- Evee Copperspring
UPDATE `creature_template` SET `subname` = 'S3竞技场商人', `npcflag`=`npcflag`|128 WHERE (`entry` = @NPC_SEASON_THREE);
SET @NPC_SEASON_FOUR         := 40207; -- Kezzik the Striker
UPDATE `creature_template` SET `subname` = 'S4竞技场商人', `npcflag`=`npcflag`|128 WHERE (`entry` = @NPC_SEASON_FOUR);
SET @NPC_ARENA_ACCESSORY     := 40209; -- Grex Brainboiler
UPDATE `creature_template` SET `subname` = '配饰商人', `npcflag`=`npcflag`|128 WHERE (`entry` = @NPC_ARENA_ACCESSORY);
